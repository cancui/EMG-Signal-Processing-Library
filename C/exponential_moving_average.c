...
struct Exponential_moving_average_ {
...
Exponential_Moving_Average *new_exponential_moving_average(uint16_t max_length_) {
	Exponential_Moving_Average *to_return;
	to_return = (Exponential_Moving_Average *) malloc(sizeof(Exponential_Moving_Average));
	if (to_return == NULL) {
...
void free_exponential_moving_average(Exponential_Moving_Average *self) {
	puts("freeing MA");
...
int get_exponential_moving_average(Exponential_Moving_Average *self, int data_entry_) {
...
int newest_entry_exponential_moving_average(Exponential_Moving_Average *self) {
	return *((int *)queue_peek_head(self->data));
}

int oldest_entry_exponential_moving_average(Exponential_Moving_Average *self){
	return *((int *)queue_peek_tail(self->data));
}

bool is_empty_exponential_moving_average(Exponential_Moving_Average *self) {
